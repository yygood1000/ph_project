...
public class OnekeyShare implements PlatformActionListener, Callback {
	...
	private boolean silent; //直接分享
	private PlatformActionListener callback;//分享操作结束后的回调
	private ShareContentCustomizeCallback customizeCallback;//为不同平台定义差别化分享内容
	private boolean dialogMode = false;//是否以dialog形式显示
	private boolean disableSSO;//设置一个总开关，用于在分享前若需要授权，则禁用sso功能
	private boolean shareVideo;//设置一个开关，用于微信分享视频
	private HashMap<String, String> hiddenPlatforms;//需要隐藏的平台集合
	private View bgView;

...
public class PlatformGridView extends LinearLayout implements
	...
	private static class GridView extends LinearLayout {
		...
		private LinearLayout getView(int position, OnClickListener ocL, Context context) {
			...
			int dp_5 = com.mob.tools.utils.R.dipToPx(context, 5);
			int dp_7 = com.mob.tools.utils.R.dipToPx(context, 7);
			int dp_40 = com.mob.tools.utils.R.dipToPx(context, 40);
			int dp_60 = com.mob.tools.utils.R.dipToPx(context, 60);
			//控件的内边距（图片和控件的距离）
			iv.setPadding(dp_7, dp_7, dp_7, dp_7);
			iv.setScaleType(ScaleType.CENTER_INSIDE);
			LayoutParams lpIv = new LayoutParams(
					LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
			//ImageView外部容器的外边距
			lpIv.setMargins(dp_5, dp_40, dp_5, dp_5);
			//设置位置
			...
			lpTv.weight = 1;
			//TextView外部容器的外边距
			lpTv.setMargins(dp_5, 0, dp_5, dp_40);

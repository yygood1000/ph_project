...
public class SwipeRefreshLayout extends ViewGroup
    ...
    private static final int DEFAULT_REFRESH_SIZE_DP = 50;
    ...
    private void initRefreshView() {
        mRefreshView = new RefreshView(getContext(), RefreshView.REFRESH);
        mRefreshView.setVisibility(View.GONE);
        if (mRefreshView instanceof IRefreshStatus) {
            mIRefreshStatus = mRefreshView;
        } else {
            throw new ClassCastException("the refreshView must implement the interface IRefreshStatus");
        }

        LayoutParams layoutParams = new LayoutParams(mRefreshViewSize * 3, mRefreshViewSize);
        addView(mRefreshView, layoutParams);
    }

    public void refreshViewSuccess() {
        mRefreshView.success();
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mRefreshView.layout((width / 2 - mRefreshView.getMeasuredWidth() / 2), (int) mRefreshInitialOffset,
                (width / 2 + mRefreshView.getMeasuredWidth() / 2), (int) (mRefreshInitialOffset + mRefreshView
                        .getMeasuredHeight()));
    }
...
//        if(mTarget.getParent() != null && !(mTarget instanceof SwipeRefreshLayout)){
//            ViewGroup group = (ViewGroup) mTarget.getParent();
...
//        }
        try {
            ...
            Logger.d("measureTarget " + e.getMessage());
        }
    }
    ...
    private int computeAnimateToRefreshingDuration(float from) {
        ...
        switch (mRefreshStyle) {
            case FLOAT:
                return (int) (Math.max(0.0f, Math.min(1.0f, Math.abs(from - mRefreshInitialOffset -
        ...
        if (!mIsRefreshing) {
            switch (mRefreshStyle) {
                case FLOAT:
                    ...
                    break;
                default:
                    convertScrollOffset = mDragDistanceConverter.convert(targetOrRefreshViewOffsetY,
                            mRefreshTargetOffset);
                    refreshTargetOffset = mRefreshTargetOffset;
                    break;
            }
    ...
    private void setTargetOrRefreshViewOffsetY(int offsetY) {
        ...
        switch (mRefreshStyle) {
            case FLOAT:
                mIRefreshStatus.pullProgress(mTargetOrRefreshViewOffsetY, (mTargetOrRefreshViewOffsetY -
                        mRefreshInitialOffset) / mRefreshTargetOffset);
                break;
            default:
                mIRefreshStatus.pullProgress(mTargetOrRefreshViewOffsetY, mTargetOrRefreshViewOffsetY /
                        mRefreshTargetOffset);

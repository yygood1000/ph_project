...
//                String nickName = EaseUserUtils.getUserInfo(message.getFrom()).getNickname();
//                digest = String.format(digest, TextUtils.isEmpty(nickName) ? message.getFrom() : nickName);
//                return digest;
//            } else {
...
//            }
        ...
        case TXT:
            ...
            if (null != txtBody && null != txtBody.getMessage()) {
                if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_VOICE_CALL, false)) {
                    ...
                    if (!StringUtils.isBlank(txtBody.getMessage())) {
                        digest = txtBody.getMessage();
                    } else {
                        digest = getString(context, R.string.dynamic_expression);
                    }
                }else if(message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_ORDER_INFO,false)){//订单详情消息
                    digest = getString(context, R.string.order);
                } else {
                    digest = txtBody.getMessage().replaceAll("\\[.{2,3}\\]", "[表情]");
                }
            }else{
                digest = "未知错误消息";
            }

...
public class EaseShowBigImageActivity extends EaseBaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		default_res = getIntent().getIntExtra("default_image", R.drawable.icon_default_avatar);
		Uri uri = getIntent().getParcelableExtra("uri");
		localFilePath = getIntent().getExtras().getString("localUrl");
		String msgId = getIntent().getExtras().getString("messageId");
//		EMLog.d(TAG, "show big msgId:" + msgId + " lacalPath " + localFilePath + uri.getPath());
	...
	private boolean savePicture() {
		...
		// 首先保存图片
		String storePath = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "560";

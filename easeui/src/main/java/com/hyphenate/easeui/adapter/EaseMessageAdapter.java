...
import com.hyphenate.easeui.widget.chatrow.EaseChatRowOrderInfo;
...
public class EaseMessageAdapter extends BaseAdapter{
	...
	private static final int MESSAGE_TYPE_SENT_ORDER_INFO = 14;
	private static final int MESSAGE_TYPE_RECV_ORDER_INFO = 15;
	...
	public int getItemViewType(int position) {
		...
		if (message.getType() == EMMessage.Type.TXT) {
		    ...
		    } else if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_ORDER_INFO,false)){ // 订单详情消息
				return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_ORDER_INFO : MESSAGE_TYPE_SENT_ORDER_INFO;
			}
	...
	protected EaseChatRow createChatRow(Context context, EMMessage message, int position) {
        ...
        case TXT:
			...
			}else if(message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_ORDER_INFO, false)){
				chatRow = new EaseChatRowOrderInfo(context,message,position,this);//交给自定义的视图处理
            }else{
    ...
    private ArrayList<ImageModel> imageMessagePaths;

	/**
	 * 添加图片地址，以及封装为预览对象实体
	 */
    public void getPhotoPath(EMMessage message){
		EMImageMessageBody imgBody = (EMImageMessageBody) message.getBody();
		if(imageMessagePaths == null){
			imageMessagePaths = new ArrayList<>();
		}
		ImageModel imageModel = new ImageModel();
		String path;
		if(!new File(imgBody.getLocalUrl()).exists()){
			path = imgBody.getRemoteUrl();
		} else {
			path = imgBody.getLocalUrl();
		}
		imageModel.setName(imgBody.getFileName());
		imageModel.setPath(path);
		imageModel.setTime(message.getMsgTime());
		imageMessagePaths.add(imageModel);
	}

	/**
	 * 返回预览图片对象集合
	 */
    public ArrayList<ImageModel> getPhotoPathList(){
    	if(imageMessagePaths == null){
    		imageMessagePaths = new ArrayList<>();
		}
		Collections.sort(imageMessagePaths, new FileUtils.FileComparator());
		return imageMessagePaths;
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		EMMessage message = getItem(position);
		if(message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_EMPTY, false)){
			return new View(context); // 空
		}
    ...
    public int getMyBubbleBg() {
        return itemStyle.getMyBubbleBg();
    }



    public int getOtherBubbleBg() {
        return itemStyle.getOtherBubbleBg();
    }

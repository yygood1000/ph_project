...
public class EaseChatRowVoicePlayClickListener implements View.OnClickListener {
	...
	public EaseChatRowVoicePlayClickListener(EMMessage message, ImageView v, ImageView iv_read_status, BaseAdapter adapter, Activity context, int position) {
				...
				public void onCompletion(MediaPlayer mp) {
					...
					if(emMessages != null && emMessages.size() > 0) {
						...
						if (message.equals(emMessage_last)) {   //  判断如果是最后一条语音则不做处理
							...
							EMMessage nextMessage = emMessages.get(position + 1);
							if (nextMessage != null) {
								int type = nextMessage.getType().ordinal();
								if (type == EMMessage.Type.VOICE.ordinal() && // 判断是否是接收到语音
										nextMessage.direct() == EMMessage.Direct.RECEIVE &&
										!nextMessage.isAcked()) {  //是否已读
									EaseChatRowVoice easeChatRowVoice = new EaseChatRowVoice(activity, nextMessage, position + 1, adapter);
